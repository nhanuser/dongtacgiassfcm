...
class FCM1():
    ...
    def read_data(self, data,label,au):
        self.data = data
    ...
    def preprocess_data(self):
        dt=self.data
        self.value= dt
        self.label = self.label_temp
        self.label_list = pd.unique(self.label)
        self.label_list = self.label_list.tolist()
        self.num_class = 2
        self.label_data =self.label.values.tolist()
        self.label_count = np.array(self.label.value_counts())
    ...
    def generate_sample(self):
        ...
        self.index_test_sample = np.where(self.label==1)[0]
        return self.index_test_sample
    def generate_U_ngang(self, k1): #k1: Số lượng phần tử được chọn làm phần tử được giám sát từ bộ dữ liệu ban đầu
        
        self.U_ngang = np.zeros((self.n,self.c))
    
        k= self.generate_sample()
        k_count= len(k)
        # self.index_x_giamsat = random.sample(range(self.n),k1)# CHọn k1 vector x từ n vector x ban đầu  
        self.index_x_giamsat= k
    ...
    def count_class(self, num_class, num_cluster):
        ...
        for k in range(self.n):
            ...
            self.cluster[k] =index_max
            # self.count_class_cluster[k_class][index_max] +=1 
        # print(self.count_class_cluster)     
    ...
    def gannhantest(self, data):  # data la du lieu
        n = data.shape[0]
        self.test_cluster=np.zeros((n))
        for k in range(n):
    
            index_max = np.argmax(data[k])
            self.test_cluster[k] =index_max
    def predict(self, new_sample):
        #Kiem tra tam cum
        if self.U is None or self.V is None:
            raise ValueError("Model has not been trained. Please run the training method first.")

        # Chuyển đổi mẫu mới thành một mảng numpy nếu cần
        if not isinstance(new_sample, np.ndarray):
            new_sample = np.array(new_sample)
        n = new_sample.shape[0]
        # Tính ma trận U cho mẫu mới
        D = np.zeros((n,self.c))

        for k in range(n):
            for i in range(self.c): 
                D[k][i] = pow(self.d_ki(k,i),1/(1-2))    
            D[k] = D[k]/sum(D[k])    
        self.gannhantest(D)
  
        return self.test_cluster  # tra ve cum
